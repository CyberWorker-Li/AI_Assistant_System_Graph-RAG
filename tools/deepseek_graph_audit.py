...
def _get_deepseek_config() -> tuple[str, str, str]:
    ...
    if not base_url or not model or not api_key:
        ...
        if not base_url:
            base_url = _read_bat_var(bat_path, "AI_ASSISTANT_POLISH_BASE_URL")
        if not model:
            model = _read_bat_var(bat_path, "AI_ASSISTANT_POLISH_MODEL")
        if not api_key:
            api_key = _read_bat_var(bat_path, "AI_ASSISTANT_POLISH_API_KEY")
        if not api_key:
            api_key = _read_bat_var(bat_path, "AI_ASSISTANT_LLM_API_KEY")
...
def _select_edges(session, limit: int, min_support: int) -> List[EdgeRow]:
    cypher = (
        ...
        "ORDER BY score DESC, lp DESC, coalesce(r.support,1) DESC "
        "LIMIT $limit"
    )
    rows = []
...
def main() -> None:
    ...
    if not api_key:
        raise SystemExit("DeepSeek API key not found. Set AI_ASSISTANT_POLISH_API_KEY or keep it in run.bat.")
    ...
    try:
        ...
        system_prompt = (
            "你是人工智能基础课程的知识图谱审查专家。"
            "输入是一批三元组(主语,关系,宾语)。请判断每条是否合理、关系名是否需要规范化、是否明显错误。"
            "只输出JSON对象，格式: "
            "{\"items\":[{\"idx\":1,\"verdict\":\"keep|reject|edit\",\"confidence\":0.0-1.0,"
            "\"suggested_label\":\"...\",\"reason\":\"...\"}]}。"
            "判定规则：不确定就keep；reject只用于明显错误/无意义/不可能关系；"
            "edit仅用于‘关系名过长且像一句话’，将其规范化成2-8个汉字的短关系名。"
            "edit必须满足 suggested_label 与原关系不同，且至少短2个字符；"
            "不要把关系改成‘通过/要求/作为/应提前/将会受到/另一方面/以/将’这类空泛词。"
        )
